...
int main(int argc, char **argv) {
  ...
  while (iteration--) {       // Compute with up, left, right, down points
    ...
    if(cpu_number==1){
        ...
        if(rank_number==0){
            MPI_Isend(vector_swap_backword,1,MPI_INT,rank_number+1,tag,MPI_COMM_WORLD,&request);
            MPI_Irecv(read_buf_back,1,MPI_INT,rank_number+1,tag,MPI_COMM_WORLD,&request);
            for(int i=0;i<W;i++)
                next[(local_l*rank_number+local_l)*W+i] = read_buf_back[i];
        }else if(rank_number>0 && rank_number<cpu_number){
            MPI_Isend(vector_swap_forward,1,MPI_INT,rank_number-1,tag,MPI_COMM_WORLD,&request);
            MPI_Isend(vector_swap_backword,1,MPI_INT,rank_number+1,tag,MPI_COMM_WORLD,&request);
            MPI_Irecv(read_buf_back,1,MPI_INT,rank_number+1,tag,MPI_COMM_WORLD,&request);
            MPI_Irecv(read_buf_front,1,MPI_INT,rank_number-1,tag,MPI_COMM_WORLD,&request);
